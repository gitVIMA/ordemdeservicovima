...
const Orders = () => {
// Adicione um estado para rastrear os itens selecionados
...
// Toggle entre selecionado e não selecionado
...
// Obtenha os endereços dos itens selecionados
const selectedOrders = orders.filter((order) => selectedItems.includes(order.id));
const selectedAddresses = selectedOrders.map((order) => order.endereco);

// Verifique se há endereços para gerar a rota
if (selectedAddresses.length === 0) {
alert('Selecione pelo menos um item para gerar a rota.');
return;
}

// Obtenha a localização atual do usuário usando a API de geolocalização do navegador
navigator.geolocation.getCurrentPosition(
(position) => {
const userLocation = new google.maps.LatLng(position.coords.latitude, position.coords.longitude);

// Crie um objeto de serviço de direções do Google Maps
const directionsService = new google.maps.DirectionsService();

// Configure as opções da rota
const routeOptions = {
  origin: userLocation,
  destination: userLocation, // Apenas para inicialização, será substituído pelo primeiro destino
  waypoints: selectedAddresses.map((address) => ({ location: address, stopover: true })),
...
directionsService.route(routeOptions, (result, status) => {
  if (status === google.maps.DirectionsStatus.OK) {
    // Abra a nova janela com a rota no Google Maps
    window.open(result.request.url, '_blank');
  } else {
...
alert('Erro ao obter a localização. Certifique-se de permitir o acesso à sua localização.');
}
);
};
...
{/* Adicione um checkbox e um evento onChange */}
<Checkbox
  checked={selectedItems.includes(order.id)}
  onChange={() => handleCheckboxChange(order.id)}
/>
<OrderCard order={order} onEditFormulario={handleEditFormulario} />
</Grid>
